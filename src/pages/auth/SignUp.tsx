...
import { Eye, EyeOff, UserPlus, Sparkle } from "lucide-react";
    ...
    <div className="min-h-screen flex flex-col items-center justify-center relative bg-blue-gradient p-4 overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
        <div className="hero-card bg-white">
          {/* Colorful top header */}
          <div className="px-8 pt-8 pb-4 bg-diwali-gradient">
            <div className="text-center">
              {/* Animated coin symbol */}
              <div className="coin mx-auto mb-4 animate-float">₹</div>
              <h2 className="text-2xl font-bold text-white">
                {translate('welcomeTitle', DEFAULT_LANGUAGE)}
              </h2>
              <p className="text-white/80 mt-1">
                {translate('welcomeMessage', DEFAULT_LANGUAGE)}
              </p>
              <p className="text-xs text-white/70 mt-2">
                {translate('freeAccess', DEFAULT_LANGUAGE)}
              </p>
            </div>
          </div>
          ...
          <div className="p-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="first-name" className="text-gray-700 font-medium">
                    {translate('firstName', DEFAULT_LANGUAGE)}
                  </Label>
                  <Input 
                    id="first-name" 
                    type="text"
                    ...
                    required 
                    className="h-12 rounded-xl border-2 border-gray-300 focus:border-coin-purple"
                  />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="last-name" className="text-gray-700 font-medium">
                    {translate('lastName', DEFAULT_LANGUAGE)}
                  </Label>
                  <Input 
                    id="last-name" 
                    type="text"
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">हम आपको एक सत्यापन कोड भेजेंगे</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input 
                    ...
                    placeholder="अपना पासवर्ड बनाएं" 
                    value={password}
              ...
              <div className="flex items-center space-x-2 bg-coin-yellow/20 p-3 rounded-lg">
                <Checkbox 
                  id="terms" 
                  checked={termsAccepted}
                  onCheckedChange={(checked) => setTermsAccepted(checked as boolean)}
                  className="data-[state=checked]:bg-coin-orange data-[state=checked]:border-coin-orange"
                  required
                />
                <Label htmlFor="terms" className="text-sm cursor-pointer">
                  {translate('termsAccepted', DEFAULT_LANGUAGE)}
                </Label>
              </div>
              ...
              >
                {loading ? (
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <span>साइन अप हो रहा है...</span>
                  </div>
                ) : (
                  <div className="flex items-center justify-center space-x-2">
                    <UserPlus className="h-5 w-5" />
                    <span>{translate('createAccount', DEFAULT_LANGUAGE)}</span>
                  </div>
                )}
              </Button>
            </form>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="bg-white px-2 text-muted-foreground">
                    {translate('orSignUpWith', DEFAULT_LANGUAGE)}
                  </span>
                </div>
              </div>
            ...
            <div className="mt-8 text-center">
              <p className="text-muted-foreground text-sm">
                {translate('alreadyHaveAccount', DEFAULT_LANGUAGE)}{" "}
                <Link to="/login" className="text-coin-purple font-medium hover:underline flex items-center justify-center gap-1 mt-1">
                  {translate('loginText', DEFAULT_LANGUAGE)}
